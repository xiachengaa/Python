...
def resolve_excel_to_strings(filePath, result_dir, ori_key_list):
    ...
    get_repeat_items_in_col(table, "简体中文")
...
#获取某一列中重复的元素的行数
def get_repeat_items_in_col(table, title):
    key_col_number = get_number_in_row(table, title, TITLELINE)
    key_col_list = table.col_values(key_col_number)
    col_list = []
    for i in xrange(1, len(key_col_list)):
        ori_item = key_col_list[i]
        item = ori_item.strip()
        if len(item) < 1:
            continue
        if item in col_list:
            print 'REPEAT: %s row: %i FirstShow: %i' % (item, i, key_col_list.index(ori_item))
        else:
            col_list.append(item)

#获取某一列中重复的元素的行数
def get_repeat_items_in_list(list):
    col_list = []
    for i in xrange(1, len(list)):
        ...
        if len(item) < 1:
            continue
        if item in col_list:
            print 'REPEAT: %s row: %i FirstShow: %i' % (item, i, list.index(ori_item))
        else:
            col_list.append(item)
...
#        os.mkdir(result_dir)
    ...
    get_repeat_items_in_list(ori_key_list)
